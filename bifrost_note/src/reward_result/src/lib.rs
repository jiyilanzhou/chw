...
use alloc::vec::Vec;
use frame_support::{Parameter, decl_module, decl_error, decl_storage};
...
impl<T: Trait> RewardTrait<T::Balance, T::AccountId> for Module<T> {
	...
	fn dispatch_reward(vtoken_symbol: TokenSymbol, staking_profit: T::Balance) -> Result<(), Error<T>> {
		// Obtain vec
		let record_vec = Self::vtoken_reward(vtoken_symbol);
		if record_vec.is_empty() {
			return Err(Error::<T>::RefererNotExist);
		}
